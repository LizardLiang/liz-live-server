...
describe("inject.client_js (TC-U07)", function()
  ...
  it("targeted-reloads on a RELOAD_PREFIX-prefixed event via maybeReload", function()
    assert.equals("reload:", inject.RELOAD_PREFIX)
    -- payload prefix is injected from the shared constant (RELOAD_PREFIX var + %q value)
    assert.truthy(js:find("RELOAD_PREFIX", 1, true))
    assert.truthy(js:find('"' .. inject.RELOAD_PREFIX .. '"', 1, true))
    assert.truthy(js:find("e.data.indexOf(RELOAD_PREFIX)", 1, true))
    assert.truthy(js:find("maybeReload", 1, true))
    assert.truthy(js:find("e.data.slice(RELOAD_PREFIX.length)", 1, true))
  end)
end)
